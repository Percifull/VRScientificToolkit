...
namespace STK
{
    ...
    public static class STKScenePlayback
    {
        ...
        public static void StartPlayback(string json, int stageToPlay) //Starts when a JSON File is loaded
        {
            ...
            playbackMode = true;
            GameObject.Find("STKTrackedObjects").GetComponent<STKTrackedObjects>().CheckForNullReferences();
            trackedObjects = GameObject.Find("STKTrackedObjects").GetComponent<STKTrackedObjects>().trackedObjects;
            parsedJson = JSON.Parse(json);
            stage = stageToPlay;
        }

        public static void DeactivateAllComponents() //Called from STKPlaybackEditor
        {
